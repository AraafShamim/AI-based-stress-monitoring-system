/**
 * National Dashboard - MannSetu
 * Functional national dashboard with real backend integration
 */
...
async function initializePage() {
    ...
    if (adminCard && user.name) {
        const strong = adminCard.querySelector('strong');
...
function setupUI() {
    ...
    // Logout
    const logoutBtn = document.querySelector('[onclick*="logout"], .logout-button');
    if (logoutBtn) {
...
function showNotification(message) {
    ...
    notification.style.cssText = 'position: fixed; top: 20px; right: 20px; background: #2196F3; color: white; padding: 15px 20px; border-radius: 4px; z-index: 10000;';
